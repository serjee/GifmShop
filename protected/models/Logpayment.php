...
/**
 * This is the model class for table "{{logpayment}}".
 *
 * The followings are the available columns in table '{{logpayment}}':
 * @property string $id
...
class Logpayment extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('currency, in_out', 'length', 'max'=>3),
			array('pay_system', 'length', 'max'=>6),
			array('payed_type', 'length', 'max'=>4),
			...
			array('id, user_id, order_id, pin_id, amount, currency, in_out, pay_system, payed_type, state, timestamp, lmi_sys_invs_no, lmi_sys_trans_no, lmi_sys_trans_date, lmi_payer_purse, lmi_payer_wm, lmi_sys_payment_id, lmi_sys_payment_date', 'safe', 'on'=>'search'),
		);
	}
