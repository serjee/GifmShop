...
class WmForm extends CFormModel
{
	...
	private $_options;
    /**
     * Тип кошелька на который производится оплата
     * @var array
     */
    private $paySys = 'WMR';
	...
	*/
    ...
    {
        // Определяем тип кошелька на который оплачивают
        if ($this->LMI_PAYEE_PURSE{0} == 'Z')
        {
            $this->paySys = 'WMZ';
        }
        // Параметры кошелька продавца
        ...
        $payment = Logpayment::model()->find('order_id=:order_id', array(':order_id'=>$this->LMI_PAYMENT_NO));
        if ($payment === null)
        {
        ...
        if ($this->paySys == 'WMR' && $payment->amount !== $this->LMI_PAYMENT_AMOUNT)
        {
            $this->addError('LMI_PAYMENT_AMOUNT', $this->paySys.' | '.$this->LMI_PAYEE_PURSE{0}.'| Сумма платежа '.$this->LMI_PAYMENT_AMOUNT.' не соотвествует сумме '.$payment->amount.' указанной при оформлении заказа.');
            return false;
        }
        if ($this->paySys == 'WMZ' && $payment->amount_usd !== $this->LMI_PAYMENT_AMOUNT)
        {
            $this->addError('LMI_PAYMENT_AMOUNT', 'Сумма платежа '.$this->LMI_PAYMENT_AMOUNT.' не соотвествует сумме '.$payment->amount_usd.' указанной при оформлении заказа.');
            return false;
        }
    }
